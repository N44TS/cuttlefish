...
# Resolver at registration: zero (working example). We set real resolver in provision step.
...
def provision_ens_identity(
    ...
    Wallet must own the ENS name. Returns (True, ens_name) or (False, error_message).
    """
    w3 = _connect(rpc_url)
    ...
    if not owner or owner.lower() != wallet.address.lower():
        return False, f"Wallet does not own '{ens_name}'."
    ...
    if not resolver_addr or resolver_addr == "0x0000000000000000000000000000000000000000":
        ...
        w3.eth.wait_for_transaction_receipt(set_resolver_tx_hash)
