/**
 * Step 2b: Auth request + reply to auth_challenge with auth_verify.
 * Same as 2a but when we see auth_challenge we sign and send auth_verify.
 *
 * Success = we see "Authenticated" in the log (server sent auth_verify).
 *
 * Run: npm run step2b
...
async function main() {
  ...
  console.log("Step 2b: Connect, send auth request, reply to auth_challenge\n");
  ...
  ws.on("message", async (data: WebSocket.Data) => {
    ...
    try {
      ...
      if (method !== "assets") console.log(JSON.stringify(parsed, null, 2));
    } catch (e) {
